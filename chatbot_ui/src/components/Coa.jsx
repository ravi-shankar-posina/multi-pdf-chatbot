...
const Coa = () => {
    ...
    useEffect(() => {
        ...
        if (stepElements.length > 0) {
            stepElements[stepElements.length - 1].scrollIntoView({ behavior: 'smooth', block: 'end' });
        }
    }, [rightSteps]);
    ...
    useEffect(() => {
        ...
        timeoutSequence.forEach(({ delay, msg }) => {
            totalDelay += delay;
            timers.push(setTimeout(() => setMessages((prev) => [...prev, msg]), totalDelay));
        });

        rightMessages.forEach((step, i) => {
            const extraDelay = i === rightMessages.length - 1 ? 10000 : 5000;
            totalDelay += extraDelay;
            timers.push(setTimeout(() => setRightSteps((prev) => [...prev, step]), totalDelay));
        });
        ...
        <div className="h-full flex flex-col bg-gray-50">
            <div className="flex flex-1 p-4 gap-4 overflow-hidden">
                <div className="w-2/3 flex flex-col bg-white shadow rounded-lg overflow-hidden border border-gray-200">
                    <div className="p-3 bg-gray-100 border-b font-medium text-gray-700 flex justify-between items-center">
                    ...
                    <div
                        className="mt-2 p-3 border-t bg-gray-50 max-h-40 overflow-y-auto"
                        ...
                        {messages.length > 0 ? (
                            <div className="space-y-2">
                                {messages.map((msg, idx) => (
                                    <div
                                        key={idx}
                                        className={`message-item p-2 rounded ${msg === "Process Successful" ? "bg-green-100 text-green-800 font-semibold" : "bg-blue-50 text-blue-700"}`}
                                    >
                            ...
                            <div className="text-gray-500 text-sm italic">Processing messages will appear here</div>
                        )}
                    </div>
                </div>
                    ...
                    >
                        {rightSteps.length > 0 ? (
                            rightSteps.map((step, idx) => (
                                <div
                                    key={idx}
                                    className="step-item flex items-center p-2 border-b border-gray-100"
                                >
